...
public class QueryDatabaseTable extends AbstractDatabaseFetchProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSessionFactory sessionFactory) throws ProcessException {
        ...
        final JdbcCommon.AvroConversionOptions options = JdbcCommon.AvroConversionOptions.builder()
                .recordName(tableName) //记录名
                .maxRows(maxRowsPerFlowFile) //最大行数
            ...
            try {
                ...
                while(true) {
                    ...
                    if (nrOfRows.get() > 0) {
                        ...
                        fileToProcess = session.putAttribute(fileToProcess, RESULT_ROW_COUNT, String.valueOf(nrOfRows.get())); //行数
                        fileToProcess = session.putAttribute(fileToProcess, RESULT_TABLENAME, tableName); //表名
                        fileToProcess = session.putAttribute(fileToProcess, CoreAttributes.MIME_TYPE.key(), JdbcCommon.MIME_TYPE_AVRO_BINARY); //mime type也就是文件类型
                        if(maxRowsPerFlowFile > 0) {
                            fileToProcess = session.putAttribute(fileToProcess, "fragment.identifier", fragmentIdentifier);//UUID
                            fileToProcess = session.putAttribute(fileToProcess, "fragment.index", String.valueOf(fragmentIndex));//碎片数
                        }
                ...
                for (int i = 0; i < resultSetFlowFiles.size(); i++) {
                    ...
                    for (Map.Entry<String, String> entry : statePropertyMap.entrySet()) {
                        ...
                        //colName = NAMESPACE_DELIMITER + NAMESPACE_DELIMITER.LENGTH
                        //NAMESPACE_DELIMITER = @!@
            ...
            try {
                ...
                if (nrOfColumns > 0) {//valid
                    for (int i = 1; i <= nrOfColumns; i++) {
                        ...
                        //columnTypeMap IS SPECIFIED WHEN YOU DEFINED THE COLUMN MAX-VALUE YOU WANNA GET
